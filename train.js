(function () {
  ...
  class NN {
    ...
    static fromJSON(obj) { return new NN(obj.w1, obj.w2); }
  ...
  if (window.DINO_SEED) {
    try {
      seedBrain = NN.fromJSON(window.DINO_SEED.brain);
      startGeneration = window.DINO_SEED.generation || 1;
      console.log('%cKayıtlı beyin yüklendi! Nesil ' + startGeneration + '\'den devam ediliyor.', 'color:blue;font-weight:bold;');
    } catch (e) {
      console.warn('DINO_SEED okunamadı, rastgele başlanıyor.', e);
    }
  }

  let population = seedBrain
    ? Array.from({ length: POP }, (_, i) => {
        if (i === 0) return seedBrain.copy();
        const c = seedBrain.copy();
        c.mutate(0.2, 0.4);
        return c;
      })
    : Array.from({ length: POP }, () => new NN());
  ...
  let hallOfFame = seedBrain ? seedBrain.copy() : null;
  let bestScoreEver = 0;
  ...
  window.dinoAI = {
    ...
    copyToClipboard: () => {
      const str = JSON.stringify(currentSaveObject());
      if (window.copy) { window.copy(str); console.log('Kopyalandı! (DevTools copy())'); }
  ...
  function updateHud() {
    saveBtn.remove();
    hud.innerHTML = `Nesil: ${generation}<br>Birey: ${genIndex + 1}/${POP} (deneme ${trialCount[genIndex] + 1}/${TRIALS_PER_GENOME})<br>Mutasyon: ${mutationRate.toFixed(3)}<br>En iyi skor (hiç): ${Math.floor(bestScoreEver)}`;
    hud.appendChild(saveBtn);
  }
  updateHud();
  ...
  function think() {
    ...
    const [jumpProb, duckProb] = population[genIndex].predict([distance, width, obsY, speed, airborne]);

    if (jumpProb > 0.5 && !trex.jumping) {
      trex.startJump(runner.currentSpeed);
    } else if (duckProb > 0.5 && !trex.jumping) {
      trex.setDuck(true);
    } else {
      trex.setDuck(false);
    }
  ...
  requestAnimationFrame(gameLoop);
  console.log('%cDino AI v3 başlatıldı (kaydet/yükle destekli)', 'color:green;font-weight:bold;');
  console.log('Manuel kaydetmek için: dinoAI.download()');
})();